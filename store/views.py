...
class ProductView(View):
    def get(self, request):
        ...
        return render(request, 'store/home.html', {'topwears': topwears, 'bottomwears': bottomwears, 'mobiles': mobiles, 'accessories':accessories, 'totalitem': totalitem})
...
def mobile(request, data=None):
    ...
    return render(request, 'store/mobile.html', {'mobiles': mobiles, 'totalitem': totalitem})


def topwear(request, data=None):
    if data == None:
        topwears = Product.objects.filter(category='TW')
    elif data == 'polo' or data == 'Samsung':
        ...
        topwears = Product.objects.filter(
            category='TW').filter(brand=data)
    return render(request, 'store/topwear.html', {'topwears': topwears})


def bottomwear(request, data=None):
    if data == None:
        bottomwears = Product.objects.filter(category='BW')
    elif data == 'lee' or data == 'Samsung':
        ...
        bottomwears = Product.objects.filter(
            category='BW').filter(brand=data)
    return render(request, 'store/bottomwear.html', {'bottomwears': bottomwears})
...
def accessorie(request, data=None):
    if data == None:
        ...
        accessories = Product.objects.filter(
            category='A').filter(discounted_price__lt=10000)
    elif data == 'above':
        accessories = Product.objects.filter(
            category='A').filter(discounted_price__gt=10000)
    return render(request, 'store/accessories.html', {'accessories': accessories})
...
def show_cart(request):
    ...
    if request.user.is_authenticated:
        ...
        if cart_product:
            ...
            return render(request, 'store/emptycart.html', {'totalitem': totalitem})
    else:
        return render(request, 'store/emptycart.html', {'totalitem': totalitem})
...
def checkout(request):
    ...
    return render(request, 'store/checkout.html', {'add': add, 'cart_items': cart_items})
...
def orders(request):
    ...
    return render(request, 'store/orders.html', {'order_placed': op})
